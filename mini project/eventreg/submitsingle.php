...
// Check if the user has already registered maximum allowed single events
$registeredSingleEventsQuery = "SELECT COUNT(*) AS total FROM eventreg WHERE username = '$uname' AND event_category = 'single'";
$registeredSingleEventsResult = $conn->query($registeredSingleEventsQuery);
$registeredSingleEventsCount = $registeredSingleEventsResult->fetch_assoc()['total'];

if ($registeredSingleEventsCount >= 5) {
    // User has already registered maximum allowed single events
    header("Location: singleform.php?error=MaxSingleEventLimitReached");
    exit();
}

// Check if the user has already registered for the same event
$checkEventQuery = "SELECT * FROM eventreg WHERE username = '$uname' AND event_name = '$eventName' AND event_category = 'single'";
$checkEventResult = $conn->query($checkEventQuery);

if ($checkEventResult->num_rows > 0) {
    // User has already registered for the same event
    header("Location: singleform.php?error=AlreadyRegistered");
    exit();
}
